...
投稿する記事の種類に応じて，プリアンブルでつぎの指定をして下さい．
\begin{itemize}
\item 論文： \\ 
\verb+\ForSubmission+, \verb+\Transaction+ 
\item 研究速報： \\
\verb+\ForSubmission+, \verb+\Transaction+, \verb+\Shortpaper+ 
\end{itemize}

論文，研究速報のどちらの場合も \verb+\Transaction+ を
プリアンブルで指定してください．
研究速報の場合は \verb+\Shortpaper+ も指定してください．
また，投稿時には必ず\verb+\ForSubmission+を指定してください．

\verb+\Transaction+ を指定することによって，
タイトルのレイアウト，図表の番号の付き方が
論文誌の執筆要項に準拠したものになります
（たとえば論文誌では図表番号は，Fig.~1, Table~1などとなります）．
また，\verb+\Shortpaper+を指定すると，
アブストラクトと著者略歴が出力されません．

\verb+\ForSubmission+ を指定すると，
本文の前（0ページ）に和文論文要旨が出力されます．
また，巻，号，発行年がそれぞれ，0, 0, 0000となります．
...
つぎに，
写真を掲載する場合は \verb+\authorbiography+を，
掲載しない場合は \verb+\authorbiography*+を
使用して各著者の情報を記載します．
\verb+\authorbiography(もしくは \authorbiography*){+ 著者名 \verb+}+ \verb+{+ ふりがな \verb+}+
